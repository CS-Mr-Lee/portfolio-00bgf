...
public class Human {
  ...
  */
  public String toString(){
    String attributes = (this.name + "\n" + this.weight + "\n" + this.energyLevel);
    return attributes;
  }
  
  /**
  Human eats a vegetable, gains the weight eaten, and gains 1% energy for every 15 cals eaten. In the 
  case that the human tries to eat more veg than exists, a slightly threatening message is printed.
  ...
  */
  public void eat(Vegetable veg, double grams){
    if (grams <= veg.getWeight()){
      ...
      this.energyLevel = (int)(this.energyLevel + (grams/veg.getWeight()*veg.getCalories())/15);
    }
    else {
      System.out.println("Stop trying to be jesus you don't have that much food");
    }
  }
  
  /**
  Human eats a cookie, gains the weight eaten, and gains 1% energy for every 15 cals eaten. In the 
  case that the human tries to eat more cookie than exists, a normal message is printed.
  Same with if the human tries to eat a cookie in the bag
  ...
  */
  public void eat(Cookie food, double grams){
    ...
    if (food.getIsPackaged() == true){
      System.out.println("Can't eat the bag");
    }
      ...
      this.weight = this.weight + (grams/1000);
      this.energyLevel = (int)(this.energyLevel + (grams/food.getWeight()*food.getCalories())/15);
    }
  }
}
